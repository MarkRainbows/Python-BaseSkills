...
print(music.count("s")) # 统计字符串中有多少个指定的字符,返回数字
sony = "abct\td"
print(sony.isalpha()) # 判断该字符是否全部为字母
print(sony.endswith("d")) # 判断该字符串是否是用指定的字符结尾，返回bool值
print(sony.startswith("ab")) # 判断该字符串是否用指定的字符开始，返回bool值
print(sony.expandtabs(tabsize=9)) # 在字符串\t 中添加指定的制表符的宽度 
...
print(read.isalnum())# 当字符串中是否全是数字或者字母 是就返回为Ture   当含有符号时就不管用"123...." ==  false
...
print(cos.ljust(50,"-"))#字符左对齐填充
print(cos.rjust(50,"+"))#字符右对齐填充
print(cos.replace("K","king"))#替换字符
print(cos.rfind("i")) # 查找字符从右边开始  但是返回下标还是从左往右
print(cos.rindex("0")) # 查找字符从右边开始  但是下标不变还是从左往右
print(cos.rstrip()) #去掉最右边的空格和换行 
print(cos.lstrip()) #去掉最左边的空格和换行 
print(cos.strip()) #去掉两边的空格和换行  
print('agsk*shabkk*habba'.split('*')) #  将字符串按照指定的字符切割成列表 , 效果 ['agsk', 'shabkk', 'habba']
